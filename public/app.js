...
async function submitRegister() {
  ...
  const pid  = document.getElementById('reg-playerid').value.trim()
  if (!user) { setAuthError('Enter a username'); return }
  if (!pass || pass.length < 6) { setAuthError('Password must be at least 6 characters'); return }
  if (!pid)  { setAuthError('Enter your WhatsApp number (e.g. 234xxxxxxxxxx)'); return }

  setAuthLoading(btn, true); clearAuthError()
  try {
    const data = await post('/api/auth/register', { username: user, password: pass, playerId: pid })
    localStorage.setItem('astral_session', JSON.stringify({ username: user, password: pass }))
    toast('Account created! Welcome, ' + (data.player?.name || user) + '! ⚔️')
    currentUser = data.player
    render(data.player)
    hideAuth()
  } catch(e) {
...
function showRegister() {
  ...
  const s = document.getElementById('step-register')
...
Object.assign(window, {
  ...
  submitLogin, submitRegister, submitReset, showForgot, showLogin, showRegister,
...
/* ══════════════════════════════════
   BILLING TOGGLE
══════════════════════════════════ */
function copyBuyCmd(cmd) {
  navigator.clipboard.writeText(cmd).catch(() => {
    const ta = document.createElement('textarea')
    ta.value = cmd; document.body.appendChild(ta); ta.select()
    document.execCommand('copy'); document.body.removeChild(ta)
  })
  toast('Copied! Send  ' + cmd + '  in WhatsApp ✓')
}
...
Object.assign(window, {
  switchPremTab, switchTopupTab, switchTermsTab, setBilling, copyBuyCmd, revealNewContent
})
